...
export default function notification({ Img, Header, Body }) {
    ...
    const placeholderImg = Img ?? "/images/face10.png"
    const placeholderHeader = Header ?? "No Header"
    const placeholderBody = Body ?? "Placeholder No Body"
    return (
        <div className='flex flex-row gap-[16px] justify-start items-start justify-items-start min-h-fit w-full max-w-xl'>
            <div className="">
                <Image
                    src={placeholderImg}
                    alt="My photo"
                    width={40}
                    height={40}
                    className='rounded-full overflow-hidden max-h-[50px] min-w-[50px] object-cover'
                />
            </div>
